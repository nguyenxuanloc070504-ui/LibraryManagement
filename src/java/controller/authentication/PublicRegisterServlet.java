...
@WebServlet(name = "PublicRegisterServlet", urlPatterns = {"/authentication/register", "/register"})
public class PublicRegisterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isEmpty(email)) {
            ...
            MemberDAO checkDao = new MemberDAO();
            try {
                if (checkDao.emailExists(email.trim())) {
                    errors.put("email", "Email already exists.");
                }
            } catch (SQLException e) {
                errors.put("email", "Error checking email: " + e.getMessage());
            } finally {
                checkDao.close();
            }
        }
